...
def generate_conv_net_base(
    ...
    if nb_layers > 1:
        model.add(Conv2D(nb_filters * 2, (3, 3), padding='same', activation='relu'))
        model.add(Conv2D(nb_filters * 2, (3, 3), padding='same', activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Dropout(0.25))

    if nb_layers > 2:
        model.add(Conv2D(nb_filters * 4, (3, 3), padding='same', activation='relu'))
        model.add(Conv2D(nb_filters * 4, (3, 3), padding='same', activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Dropout(0.25))
...
def generate_vgg16_conv(input_shape, full_16=False, dropout=True):
    ...
    model.add(Conv2D(128, (3, 3), activation='relu', padding=padding))
    model.add(Conv2D(128, (3, 3), activation='relu', padding=padding))
    model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
    if dropout:
        model.add(Dropout(0.25))

    model.add(Conv2D(256, (3, 3), activation='relu', padding=padding))
    model.add(Conv2D(256, (3, 3), activation='relu', padding=padding))
    model.add(Conv2D(256, (3, 3), activation='relu', padding=padding))
    model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
    if dropout:
    ...
    if full_16:
        for i in xrange(2):
            model.add(Conv2D(512, (3, 3), activation='relu', padding=padding))
            model.add(Conv2D(512, (3, 3), activation='relu', padding=padding))
            model.add(Conv2D(512, (3, 3), activation='relu', padding=padding))
            model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
            if dropout:
